...
import { postMeal, editMeal } from "@/lib/api-calling-methods/mealBackendMethods.ts";
...
const LogMealForm = ({ setOpen, meal }: LogMealFormProps) => {
    ...
    const { mutateAsync: logMealMutation, isPending } = useMutation<
        ...
        MealData
    >({
        mutationFn: (mealData: MealData) => mutationFunction(mealData),
        onSuccess: () => {
    ...
    const onSubmit = async (data: MealData) => { // Type data as MealData
        try {
            const dataToSubmit = isEditMode ? { ...data, id: meal?.id } : data;
            await logMealMutation(dataToSubmit);
        } catch (error) {
